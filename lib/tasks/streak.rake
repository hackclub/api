namespace :streak do
  ...
  desc "View the stages and their associated keys for a given pipeline. Use this to figure out the stage mappings for a pipeline."
  task :debug_stages, [:pipeline_key] => :environment do |t, args|
    p = StreakClient::Pipeline.find(args[:pipeline_key])
